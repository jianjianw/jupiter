...
public class InvalidReasonReportsDao {
     ...
     * @return
     */
    public List<Map<String, Object>> getInvalidReasonReports(List<DictionaryPO> list, Integer companyId, String sourceIds, String startTime, String endTime, String typeIds) {
        StringBuilder sql = new StringBuilder();
        sql.append("SELECT src.SRCNAME,src.SRCIMG");
        sql.append(",(SELECT count(detail.ID) FROM hm_crm_client_detail detail LEFT JOIN hm_crm_client_info info ON info.KZID = detail.KZID WHERE src.ID = info.SOURCEID");
        ...
        hjsql.append(",(SELECT count(detail.ID) FROM hm_crm_client_detail detail LEFT JOIN hm_crm_client_info info ON detail.KZID=info.KZID ");
        if (StringUtil.isNotEmpty(typeIds)) {
