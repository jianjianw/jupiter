...
//                OSSUtil.putOssObject(fileName, file.getInputStream());
                path = OSSUtil.upload(file.getInputStream(), null, file.getOriginalFilename().substring(file.getOriginalFilename().lastIndexOf(".")));
            } catch (Exception e) {
                e.printStackTrace();
        ...
        return ResultInfoUtil.success(TipMsgConstant.SUCCESS, path);
    }
