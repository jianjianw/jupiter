...
public class DingAuthUtil {
	...
	public DingAuthDTO getPersistentCode(String authCode) {
		...
		String resStr = HttpClient
				// 请求方式和请求url
				.textBody(persistentCodeUrl).queryString("access_token", accessToken)
				// post提交json
				.json(params.toString()).asString();
		JSONObject res = JSON.parseObject(resStr);
	...
	public void getUserInfo(String snsToken) {
		String resStr = HttpClient.get(userInfoUrl).queryString("sns_token", snsToken).asString();
		JSONObject res = JSON.parseObject(resStr);
