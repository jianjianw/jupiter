...
//            reply.put("callback", callbackData);
    ...
    /**
     * 根据Key或者url获取文件名称
     * 如key:image/2018/05/03/180503164504_dfd59c47-a91e-4b7e-8697-59dab618db16.png
     *
     * @return
     */
...
//        objectMetadata.setHeader("Pragma", "no-cache");
     ...
     *
     * @param key          文件key值
     * @param localFileUrl 目标文件路径名称
     * @return boolean
     */
    public static boolean getObject(String key, String localFileUrl) {
        ObjectMetadata obj = client.getObjectMetadata(BUCKET_NAME, key);
        String fileName = getObjectName(key);
        File file = new File(localFileUrl);
        if (!file.exists()) {
            file.mkdirs();
        }
        ObjectMetadata object = client.getObject(new GetObjectRequest(BUCKET_NAME, key), new File(localFileUrl + fileName));
        if (object != null) {
            return true;
        }
        return false;
