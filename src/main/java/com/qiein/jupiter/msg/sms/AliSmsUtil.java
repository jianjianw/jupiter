...
//	 * @throws EduException
//	 */
//	public static synchronized void send(String telNo, SmsDTO smsDto) throws EduException {
...
//		req.setSmsFreeSignName(StringUtil.nullToStrTrim(smsDto.getSign()));// 短信签名，传入的短信签名必须是在阿里大鱼“管理中心-短信签名管理”中的可用签名。
//		req.setSmsParamString(smsDto.getContent().toString());// 短信模板变量，传参规则{"key":"value"}，key的名字须和申请模板中的变量名一致，多个变量之间以逗号隔开。
//		req.setRecNum(telNo);// 短信接收号码（短信接收号码。支持单个或多个手机号码，传入号码为11位手机号码，不能加0或+86。群发短信需传入多个号码，以英文逗号分隔，一次调用最多传入200个号码。）
//		req.setSmsTemplateCode(StringUtil.nullToStrTrim(smsDto.getTemplateId()));// 短信模板ID
//		try {
//			rsp = client.execute(req);
//			String rspRst = rsp.getBody();
//			JSONObject rstJson = JSONObject.parseObject(rspRst);
//			if (!"0".equals(rstJson.getJSONObject("alibaba_aliqin_fc_sms_num_send_response").getJSONObject("result")
//					.getString("err_code"))) {
...
//	 * @throws EduException
//	 */
//	public static synchronized void sendClientYyShop(String sign, String template, String telNo, String code,
